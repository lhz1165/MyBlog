...
public class CommentServiceImpl implements CommentService {
    ...
    /*查询个人的评论*/
    @Override
    public PageInfo<Comment> listCommentByPagOfUser(Integer pageIndex, Integer pageSize, List<Integer> articleIds) {
        PageHelper.startPage(pageIndex, pageSize);
        List<Comment> commentList = null;
        try {
            commentList = commentMapper.listCommentByUserArticle(articleIds);
            for (int i = 0; i < commentList.size(); i++) {
                Article article = articleMapper.getArticleByStatusAndId(ArticleStatus.PUBLISH.getValue(), commentList.get(i).getCommentArticleId());
                commentList.get(i).setArticle(article);
            }
