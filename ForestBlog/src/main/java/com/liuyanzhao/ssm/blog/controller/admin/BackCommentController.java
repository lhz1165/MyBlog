...
public class BackCommentController {
    ...
    public String commentListView(@RequestParam(required = false, defaultValue = "1") Integer pageIndex,
        ...
        if (user.getUserNickname().equals("admin")){
            commentPageInfo= commentService.listCommentByPage(pageIndex, pageSize);
            ...
            List<Integer> articlesIds = articleMapper.findAll(criteria).stream().map(Article::getArticleId).collect(Collectors.toList());
            commentPageInfo= commentService.listCommentByPagOfUser(pageIndex, pageSize,articlesIds);
        }
